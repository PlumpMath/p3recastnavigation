...
#include <float.h>
#include <new>
#include "NavMeshType_Obstacle.h"
							   ...
							   const int maxTiles)
{
	...
	{
		const rnsup::rcChunkyTriMeshNode& node = chunkyMesh->nodes[cid[i]];
	...
	{
		rnsup::TileCacheData* tile = &rc.tiles[rc.ntiles++];
...
class TempObstacleCreateTool : public NavMeshTypeTool
{
	NavMeshType_Obstacle* m_sample;
...
//void NavMeshType_Obstacle::handleSettings()
//{
...
//}
//
//void NavMeshType_Obstacle::handleTools()
//{
...
//}
//
//void NavMeshType_Obstacle::handleDebugMode()
//{
...
//}
//
//void NavMeshType_Obstacle::handleRenderOverlay(double* proj, double* model, int* view)
//{
//	if (m_tool)
...
void NavMeshType_Obstacle::handleMeshChanged(class InputGeom* geom)
{
...
void NavMeshType_Obstacle::removeTempObstacle(const float* sp, const float* sq)
{
	if (!m_tileCache)
		return;
...
void NavMeshType_Obstacle::clearAllTempObstacles()
{
	if (!m_tileCache)
		return;
...
bool NavMeshType_Obstacle::handleBuild()
{
	dtStatus status;
...
void NavMeshType_Obstacle::handleUpdate(const float dt)
{
	NavMeshType::handleUpdate(dt);
...
void NavMeshType_Obstacle::getTilePos(const float* pos, int& tx, int& ty)
{
	if (!m_geom) return;
...
void NavMeshType_Obstacle::loadAll(const char* path)
{
